...
def _schema() -> DatabaseSchema:
    return DatabaseSchema(tables=[
        TableInfo(name="employees", columns=[
            ColumnInfo(name="id", data_type="integer"),
            ColumnInfo(name="first_name", data_type="varchar"),
            ColumnInfo(name="national_id", data_type="varchar"),
        ]),
        TableInfo(name="test1", columns=[ColumnInfo(name="id", data_type="integer")]),
    ])
...
def test_expands_select_star_using_real_schema_columns() -> None:
    ...
    assert result.sql == "SELECT employees.id, employees.first_name, employees.national_id FROM employees"
...
def test_repairs_unique_numeric_table_suffix() -> None:
    ...
    assert result.valid is True
    assert "test1.id" in result.sql and "FROM test1" in result.sql
...
def test_quotes_unquoted_national_id() -> None:
    result = SQLRepairLoop().repair(
        ...
        _schema(), intent=QueryIntent(requested_entity="employee", national_id="1234567890"),
    )
    assert result.valid is True
...
def test_stops_when_no_schema_grounded_repair_exists() -> None:
    result = SQLRepairLoop().repair("SELECT employees.unknown_column FROM employees", _schema())
    assert result.valid is False
    assert result.stopped_reason == "no_safe_repair"


def test_returns_without_attempt_when_sql_is_already_valid() -> None:
    result = SQLRepairLoop().repair("SELECT employees.id FROM employees", _schema())
    assert result.valid is True
